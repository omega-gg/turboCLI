...
# qwen-image-edit-2511-lightning-angles -- the qwen-image-edit-2511 model with "lightning" (always) +
# "angles" (only for <sks> prompts). The angles toggle is part of the cache key (extra_key), so
# flipping it reloads the pipe. Same model/pipeline as qwen-image-edit-2511; TYPE stays "qwen-image-edit"
# so the backend seam treats it as a normal qwen-image-edit. Standalone by design.
...
# Install (python -m runner.install): the model + the lightning and angles LoRAs, into the model folder.
MODEL = {"repository": "Qwen", "model": "Qwen-Image-Edit-2511"}

# LoRAs applied on load, found inside the model folder (the same files the install fetches). "angles"
# only for <sks> prompts.
...
def loras(params):
    # Lightning always; angles (@0.9) only for <sks> prompts. Core resolves paths and adds ctx.loras.
    files = [(LIGHTNING, 1.0)]
