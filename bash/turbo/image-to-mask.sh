...
if [ $# -lt 4 -o $# -gt 6 ] \
   ...
   [ "$2" != "cpu" -a "$2" != "cuda" -a "$2" != "mps" ]; then
    ...
    echo ""
    echo "input images: separated by a comma, the input first. mask / mask-region: input,reference."
    echo "              matte engines: input, or input,plate (a plate keeps the cast shadow)."
    echo ""
    ...
    echo ""
    echo "examples:"
    echo "    image-to-mask mask          cpu  edited.png,original.png mask.png threshold=40"
